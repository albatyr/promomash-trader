...
namespace Promomash.Trader.UserAccess.Application.Countries.GetAllProvincesByCountry
{
    ...
    {
        public async Task<List<ProvinceDto>> Handle(GetAllProvincesByCountryQuery request, CancellationToken cancellationToken)
        {
            ...
            var provinces = await connection.QueryAsync<ProvinceDto>(sql, new {CountryCode = request.CountryCode});
